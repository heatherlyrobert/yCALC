...
ycalc__mock_new          (tMOCK **a_owner)
{
   ...
   --rce;  if (a_owner == NULL) {
      DEBUG_YCALC   yLOG_snote   ("no return entry pointer");
      DEBUG_YCALC   yLOG_sexitr  (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_YCALC   yLOG_spoint  (*a_owner);
   --rce;  if (*a_owner != NULL) {
      DEBUG_YCALC   yLOG_snote   ("entry pointer already assigned");
      DEBUG_YCALC   yLOG_sexitr  (__FUNCTION__, rce);
      return rce;
   }
   /*---(create)-------------------------*/
   ...
   ++myCALC.mcount;
   DEBUG_YCALC   yLOG_sint    (myCALC.mcount);
   /*---(cell pointers)------------------*/
...
ycalc__mock_wipe     (tMOCK *a_owner)
{
   ...
   DEBUG_YCALC   yLOG_spoint  (a_owner);
   --rce;  if (a_owner == NULL) {
      DEBUG_YCALC   yLOG_snote   ("no entry pointer");
      DEBUG_YCALC   yLOG_sexitr  (__FUNCTION__, rce);
      return rce;
   }
   /*---(data)---------------------------*/
   DEBUG_YCALC   yLOG_snote   ("type");
   a_owner->type   = G_DEP_BLANK;
   DEBUG_YCALC   yLOG_snote   ("source");
   if (a_owner->source    != NULL)  free (a_owner->source);
   a_owner->source         = NULL;
   DEBUG_YCALC   yLOG_snote   ("value");
   a_owner->value          =  0.0;
   DEBUG_YCALC   yLOG_snote   ("string");
   if (a_owner->string    != NULL)  free (a_owner->string);
   a_owner->string         = NULL;
   DEBUG_YCALC   yLOG_snote   ("print");
   if (a_owner->print     != NULL)  free (a_owner->print);
   a_owner->print          = NULL;
   DEBUG_YCALC   yLOG_snote   ("label");
...
ycalc__mock_free         (tMOCK **a_owner)
{
   ...
   DEBUG_YCALC   yLOG_spoint  (*a_owner);
   --rce;  if (*a_owner == NULL) {
      DEBUG_YCALC   yLOG_snote   ("entry pointer not assigned");
      DEBUG_YCALC   yLOG_sexitr  (__FUNCTION__, rce);
      return rce;
   }
   x_owner = *a_owner;
...
ycalc__mock_cleanup     (void)
{
   ...
   DEBUG_YCALC   yLOG_point   ("mhead"     , myCALC.mhead);
   DEBUG_YCALC   yLOG_point   ("mtail"     , myCALC.mtail);
   DEBUG_YCALC   yLOG_value   ("mcount"    , myCALC.mcount);
   x_owner = myCALC.mroot;
   while (x_owner != NULL) {
      x_save  = x_owner->next;
      DEBUG_YCALC   yLOG_point   ("before"     , x_owner);
      ycalc_call_reaper (&x_owner, &(x_owner->ycalc));
      ycalc__mock_free  (&x_owner);
      DEBUG_YCALC   yLOG_point   ("after"      , x_owner);
      x_owner = x_save;
   }
   myCALC.mroot = NULL;
...
ycalc__mock_enabler     (void *a_owner, void *a_deproot)
{
   ...
   DEBUG_YCALC   yLOG_spoint  (x_mock->ycalc);
...
ycalc__mock_list        (void)
{
   ...
   DEBUG_YCALC   yLOG_value   ("mcount"    , myCALC.mcount);
   while (x_owner != NULL) {
      DEBUG_YCALC   yLOG_point   ("next"      , x_owner);
      if (x_owner->label != NULL) {
         DEBUG_YCALC   yLOG_info    ("checking"  , x_owner->label);
      } else {
         DEBUG_YCALC   yLOG_info    ("checking"  , "label null");
      }
      x_owner = x_owner->next;
      ++c;
   }
...
ycalc__mock_named       (char *a_label, char a_force, void **a_owner, void **a_deproot)
{
   ...
   --rce;  if (x_owner == NULL) {
      ...
      if (myCALC.mroot == NULL && strcmp (a_label, "ROOT") == 0) {
         DEBUG_YCALC   yLOG_note    ("saved as root");
         myCALC.mroot = x_owner;
      }
   }
   --rce;  if (x_owner == NULL) {
      DEBUG_YCALC   yLOG_note    ("FAILURE");
      DEBUG_YCALC   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(handle normal)------------------*/
...
ycalc__mock_reaper      (void **a_owner)
{
   ...
   x_mock = (tMOCK *) *a_owner;
   DEBUG_YCALC   yLOG_point   ("ycalc"     , x_mock->ycalc);
   --rce;  if (x_mock->ycalc  != NULL) {
      DEBUG_YCALC   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_YCALC   yLOG_char    ("type"      , x_mock->type);
...
ycalc__mock_pointer     (void *a_owner, char **a_source, char **a_type, double **a_value, char **a_string)
{
   ...
   if (a_source != NULL) {
      ...
      *a_source = &x_mock->source;
      DEBUG_YCALC   yLOG_point   ("*a_source" , *a_source);
      DEBUG_YCALC   yLOG_point   ("->source"  , x_mock->source);
      if (x_mock->source != NULL)  DEBUG_YCALC   yLOG_info    ("->source"  , x_mock->source);
   }
   if (a_type  != NULL) {
   ...
   if (a_string != NULL) {
      ...
      *a_string = &x_mock->string;
      DEBUG_YCALC   yLOG_point   ("*a_string" , *a_string);
      DEBUG_YCALC   yLOG_point   ("->string"  , x_mock->string);
      if (x_mock->string != NULL)  DEBUG_YCALC   yLOG_info    ("->string"  , x_mock->string);
   }
   DEBUG_YCALC   yLOG_exit    (__FUNCTION__);
   return 0;
}
...
ycalc__mock_valuer      (void *a_owner, char *a_type, double *a_value, char **a_string)
{
   ...
   if (a_string != NULL) {
      ...
      DEBUG_YCALC   yLOG_spoint  (*a_string);
...
ycalc__mock_address     (void *a_owner, int *b, int *x, int *y, int *z)
{
   ...
   if (z != NULL)  *z   = 0;
   if (a_owner == NULL) {
      DEBUG_YCALC   yLOG_sexitr  (__FUNCTION__, -1);
      return -1;
   }
   x_mock    = (tMOCK     *) a_owner;
...
ycalc__mock_special     (void *a_owner, char a_what, double *a_value, char **a_string)
{
   ...
   case G_SPECIAL_SOURCE :
      DEBUG_YCALC   yLOG_snote   ("source");
      DEBUG_YCALC   yLOG_snote   (x_owner->source);
      if (a_string != NULL)  *a_string = x_owner->source;
      break;
   case G_SPECIAL_PRINT  :
      DEBUG_YCALC   yLOG_snote   ("print");
      DEBUG_YCALC   yLOG_snote   (x_owner->print);
      if (a_string != NULL)  *a_string = x_owner->print;
      break;
   case G_SPECIAL_TYPE   :
      DEBUG_YCALC   yLOG_snote   ("type");
      DEBUG_YCALC   yLOG_schar   (x_owner->type);
      if (a_value  != NULL)  *a_value  = x_owner->type;
      break;
   }
   DEBUG_YCALC   yLOG_spoint  (*a_string);
   if (*a_string != NULL)  DEBUG_YCALC   yLOG_snote   (*a_string);
   DEBUG_YCALC   yLOG_sexit   (__FUNCTION__);
   return 0;
}
...
ycalc__mock_width    (void *a_owner, int *a_width, int *a_merge)
{
   ...
   *a_merge = 0;
   ...
   for (i = x + 1; i < x + 20; ++i) {
      ...
      DEBUG_YCALC   yLOG_complex ("owner"     , "%-10p, %-5s, %3db, %3dx, %3dy, %3dz, %3dw", x_owner, x_label, b, i, y, z, x_owner->width);
      *a_width += x_owner->width;
      ++(*a_merge);
      s_owners [*a_merge] = x_owner;
      s_widths [*a_merge] = x_owner->width;
...
ycalc__mock_parse    (char *a_full, int a_merge)
{
   ...
   /*---(header)-------------------------*/
   DEBUG_YCALC   yLOG_senter  (__FUNCTION__);
   DEBUG_YCALC   yLOG_spoint  (a_full);
   if (a_full == NULL) {
      DEBUG_YCALC   yLOG_sexit   (__FUNCTION__);
      return 0;
   }
   DEBUG_YCALC   yLOG_sint    (a_merge);
   for (i = 0; i <= a_merge; ++i) {
      x_owner = s_owners [i];
      if (x_owner == NULL)  break;
      w       = s_widths [i];
      DEBUG_YCALC  yLOG_sint    (w);
      /*> printf ("malloc () ycalc__mock_parse\n");                                   <*/
      while (p == NULL)  p = (char*) malloc (w + 1);
      sprintf (p, "%-*.*s", w, w, a_full + wa);
      DEBUG_YCALC  yLOG_snote   (p);
...
ycalc__mock_source      (char *a_label, char *a_source)
{
   ...
   DEBUG_YCALC   yLOG_point   ("existing"  , x_owner->source);
   DEBUG_YCALC   yLOG_info    ("existing"  , x_owner->source);
   if (x_owner->source != NULL) {
      DEBUG_YCALC   yLOG_note    ("must free");
      free (x_owner->source);
      x_owner->source = NULL;
      DEBUG_YCALC   yLOG_point   ("freed"     , x_owner->source);
   }
   if (a_source != NULL) {
      x_owner->source = strdup (a_source);
      DEBUG_YCALC   yLOG_point   ("new"       , x_owner->source);
      DEBUG_YCALC   yLOG_info    ("new"       , x_owner->source);
   }
   DEBUG_YCALC   yLOG_exit    (__FUNCTION__);
   return 0;
}
...
ycalc__mock_whole       (char *a_label, char *a_source, char a_format, char a_decs, char a_align, char a_width)
{
   ...
   DEBUG_YCALC   yLOG_value   ("source"    , rc);
   --rce;  if (rc < 0) {
      DEBUG_YCALC   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
